...
-- Exercise 1: 1 point
theorem and_commutes {P Q : Prop} : P ∧ Q → Q ∧ P := sorry

-- Exercise 2: 1 point
theorem or_commutes {P Q : Prop} : P ∨ Q → Q ∨ P := sorry

-- Exercise 3: 1 point
theorem implication_transitive {P Q R : Prop} : (P → Q) → (Q → R) → (P → R) := sorry

-- Exercise 4: 1 point
theorem forall_conjunction {P R : Nat → Prop} : (∀ n, P n ∧ R n) → (∀ n, P n) := sorry


-- Exercise 5: 2 points
theorem maximum_associative (n m k : Nat) :
  maximum n (maximum m k) = maximum (maximum n m) k := sorry
